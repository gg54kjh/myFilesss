function Coord() {
var d=document,x,div=d.body.appendChild(d.createElement("div"));div.id="Coor";function yy(s){if(s.shiftKey){if(!d.querySelector('#coor_')){x=d.body.appendChild(div.cloneNode(true));x.id='coor_';x.className="mydiv_class"}else x.remove()}if(s.keyCode==27){div.remove();x?.remove();d.removeEventListener("mousemove",zz,!0);d.removeEventListener("keydown",yy)}};function zz(e){div.innerHTML=`<style>#Coor,#coor_{position:fixed;background:#fff;z-index:2147483647;padding:4px 8px;border:1px solid black;top:${e.clientY}px;left:${e.clientX}px</style>${e.clientX},${e.clientY}<hr>${e.pageX},${e.pageY}`}d.addEventListener("mousemove",zz,!0);d.addEventListener("keydown",yy)
}
function Vremya() {
var dc = document;
 dc.body.insertAdjacentHTML('beforeend', `
      <div id="my_vr">
          <style>#my_vr{padding:3px 10px;border:6px groove;position:fixed;top:150px; right:3px;background:azure; color:#000;
...
<style>#bookm {position:fixed; top:20px; left:650px; background:snow; z-index:20000}
       ...
       #bookm  a:hover,  #bookm  span:hover{color: #0000CD}
       ...
       #bookm span:hover {text-decoration:underline; cursor:pointer}
