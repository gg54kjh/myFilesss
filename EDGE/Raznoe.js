...
<div id="kbev" class="mydiv_class mydrg">
  <style>
       #kbev input[type="button"] {all: revert}
       #kbev {
           position:fixed; top:0; left:100px; background:white; 
           z-index:99999; border: 1px solid; padding: 4px;
        }
       #kinput {max-width: 490px; box-sizing: border-box}
       #conv {max-width: 200px;  box-sizing: border-box} 
       #area {
           width: 600px;
   ...
   <form id="form" onsubmit="return false">
      <input value="keyCode >key" style="margin-left:140px" type="button"><br>
      <input value="Hex >Dec" type="button">
      <input id="conv" style="padding: 3px 0 1px 6px" type="text">
      <input value="Dec >Hex" type="button">
      <input value="C L O S E" type="button" style="float: right"><br>
      <input value="charCode >key" style="margin-left:140px; margin-bottom:3px" type="button"> 
     <br>
    <input placeholder="Press key here!" id="kinput" size=25 type="text" autofocus="">
    <input value="Clear All" type="button">
    <textarea id="area" style="height:189px"></textarea>
  </form>
</div> `
...
st.textContent = `
#urldcdr {position: fixed; top:0; left:1px; z-Index:2147483647; background:white}
#urldcdr button:not(#btcls) {all: revert; width: 17%}
#urldcdr #btcls {all: revert; width: 48%}
#txtar {padding-left:7px; border:none; width:1140px; height:150px; background:azure; color:#000} `;
...
let bt5 = b(); bt5.innerHTML = 'RGB<>HEX'; bt5.onclick = 
()=>{ dv.remove(); runRgbHex() }
...
//bt1.style.width=bt2.style.width=bt3.style.width=bt4.style.width=bt5.style.width=clear.style.width='17%';
//btcls.style.width='48%';
dv.append(st, textarea, br(), bt1, bt2, clear, btcls, br(), bt3, bt4, bt5);
...
let s = `
  <div id="rgbconv" class="mydiv_class mydrg">
    <style>
       #rgbconv {
           position:fixed; top:0; left:350px; background:white; 
           z-index:99999; border: solid 2px; padding-right: 30px;
       }
       #rgbconv input[type="button"] {all: revert; position: relative}
       #rgbconv input[type="text"] {backgrond:#fff; color:#000}
       #rgbconv #in0 {left: 100px}
       #in1 {position: relative; width: 8rem; text-align: center}
       #rgbconv #in2 {left: 100px}
       #in4 {width: 8rem}
       #rgbconv #in5 {margin-top:3px;left: 100px}
       #colorSample {position: relative; top: 2px; display:inline-block; border: 1px solid; width: 40px}
       #shablon {position: relative; left: 0px; bottom: 0px; width: 40px; height: 25px; border: 1px solid}              
    </style>
   <p><br></p>
<form name="rgb"> 
   <input id="in0" value="hex to rgb" type="button">
  <div style="flex; margin: 3px 0 1px 0">
    <div id="colorSample">
       &nbsp;
   </div>
